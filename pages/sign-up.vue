    ...
    <div class="flex flex-col ml-32 lg:items-start items-center">
      ...
      >
        <div class="flex flex-row justify-between gap-5">
          ...
          <a-form-item class="relative text-white md:w-[45%]" :rules="[{ required: true }]">
            <a-input :allow-clear="true" placeholder="Last name"/>
            <span class="w-[20px] h-[20px] text-white absolute top-0 right-4">
              <icon-card-idetity/>
            </span>
          </a-form-item>
        ...
        <a-form-item class="relative w-full text-white" :rules="[{ required: true }]">
          <a-input :allow-clear="true" placeholder="Email"/>
          <span class="w-[20px] h-[20px] text-white absolute top-0 right-4">
            <icon-email/>
          </span>
        </a-form-item>
        <a-form-item class="relative w-full text-white" :rules="[{ required: true }]">
          <a-input :allow-clear="true" placeholder="Username" show-count/>
          <span class="w-[20px] h-[20px] text-white absolute top-0 right-4">
            <icon-user/>
          </span>
        </a-form-item>
        <a-form-item class="relative w-full text-white" :rules="[{ required: true }]">
          <a-input :type="isShowPass ? 'text' : 'password'" :allow-clear="true" show-count placeholder="Password"/>
          <span class="w-[20px] h-[20px] text-white absolute top-0 right-4" @click="showPass">
            <icon-hide-password v-if="isShowPass"/>
            <icon-password v-else/>
          </span>
        </a-form-item>
...
export default {
  ...
  methods: {
    showPass() {
      console.log(this.isShowPass, 'first')
      this.isShowPass = !this.isShowPass
      console.log(this.isShowPass, 'second')
